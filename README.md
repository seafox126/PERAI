...
## Voice mode (offline, Jarvis-style wake activation)
...
Set wake-word activation (example: Jarvis):
```bash
export WAKE_WORD=jarvis
PYTHONPATH=src python -m offline_assistant.voice --record-seconds 4 --wake-followup-turns 1
```

Wake mode behavior:
- If `WAKE_WORD` is set, speech is ignored until wake token is heard.
- `jarvis` alone triggers wake acknowledgment.
- `jarvis <command>` executes immediately.
- After `jarvis` alone, one follow-up turn is allowed without repeating wake word (configurable with `--wake-followup-turns`).
...
## Ollama setup (optional, for multi-purpose semantic fallback)
...
When deterministic rules do not match, assistant sends transcript to local Ollama and expects strict JSON intent output.
