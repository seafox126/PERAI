...
class WakeWordGate:
    """Controls whether a transcript should be processed in voice mode."""

    def __init__(self, wake_word: str | None, allow_followup_turns: int = 1) -> None:
        self.wake_word = wake_word.strip().lower() if wake_word else None
        self.allow_followup_turns = max(0, allow_followup_turns)
        self._remaining_followup_turns = 0

    def filter_transcript(self, transcript: str) -> str | None:
        ...
        if text == self.wake_word:
            self._remaining_followup_turns = self.allow_followup_turns
            return text

        if text.startswith(f"{self.wake_word} "):
            self._remaining_followup_turns = 0
            return text

        if self._remaining_followup_turns > 0:
            self._remaining_followup_turns -= 1
            return text
...
def run_voice_loop() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many post-wake turns to allow without repeating wake word",
    )
    ...
    wake_gate = WakeWordGate(wake_word=wake_word, allow_followup_turns=args.wake_followup_turns)
    ...
    while True:
        ...
        filtered = wake_gate.filter_transcript(transcript)
        if not filtered:
            print("null")
            continue

        result = assistant.handle_transcript(filtered, wake_word=wake_word)
        if result is None:
            print("null")
            continue
